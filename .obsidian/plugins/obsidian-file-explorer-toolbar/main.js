...
// 配置
const DEFAULT_SETTINGS = { scrollToFile: true, followScrollToFile: false, expandOrCollapse: true };
// 在文件列表中显示当前打开的文件
const SCROLL_TO_FILE_ICON = '<svg viewBox="0 0 100 100" class="feather-crosshair" width="18" height="18"><svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-crosshair"><circle cx="12" cy="12" r="10"></circle><line x1="22" y1="12" x2="18" y2="12"></line><line x1="6" y1="12" x2="2" y2="12"></line><line x1="12" y1="6" x2="12" y2="2"></line><line x1="12" y1="22" x2="12" y2="18"></line></svg></svg>';
...
class ProviderBase {
     ...
     */
    getButtons(leaf) {
        return leaf.view.containerEl.querySelectorAll('.file-explorer-toolbar-button');
    }
}
...
class ScrollToFileProvider extends ProviderBase {
    ...
    // 执行命令
    executeCommand() {
        this.plugin.app.commands.executeCommandById('file-explorer:reveal-active-file');
    }
}
...
class ExpandOrCollapseProvider extends ProviderBase {
    ...
    // get the button
    getButton(leaf) {
        const buttons = leaf.view.containerEl.querySelectorAll('.expand-or-collapse');
        if (buttons && buttons.length > 0) {
            return buttons[0];
        }
        return null;
    ...
    executeCommand(leaf, button) {
        ...
        const items = this.getExplorerItems(leaf);
        items.forEach((item) => {
            if (this.explorerItemIsFolder(item) && item.collapsed !== this.isCollapsed) {
...
class FileExplorerToolbarPluginSettings extends obsidian.PluginSettingTab {
    ...
    display() {
        ...
        new obsidian.Setting(this.containerEl)
            ...
            .setDesc('如果启动，将会在文件资源管理器上方的工具栏中添加【在文件列表中显示当前文件】功能。')
            .addToggle((toggle) => {
            toggle
        ...
        new obsidian.Setting(this.containerEl)
            ...
            new obsidian.Setting(this.containerEl)
            .setName('展开/关闭全部文件夹')
            .setDesc('如果启动，将会在文件资源管理器上方的工具栏中添加【展开/关闭所有文件夹】功能。')
            .addToggle((toggle) => {
            toggle
